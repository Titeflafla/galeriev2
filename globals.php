...
foreach ($_FILES as $k => $v) {
    if(!empty($_FILES[$k]['name'])) {
        ...
        $nbUpload = count($_FILES[$k]['name']);
        // Pour chaque fichier on fait les vérification
        for ($f = 0;$f < $nbUpload;$f++) {
            $_FILES[$k]['name'][$f] = substr(md5(uniqid()), rand(0, 20), 10) . strrchr($_FILES[$k]['name'][$f], '.');
            $sfile = new finfo(FILEINFO_MIME);
            if (stripos(strrchr($_FILES[$k]['name'][$f], '.'), 'php') !== false || stripos($sfile->file($_FILES[$k]['tmp_name'][$f]), 'php') !== false) {
                die ('Upload a PHP file isn\'t autorised !!');
            }
            elseif (stripos(strrchr($_FILES[$k]['name'][$f], '.'), 'htaccess') !== false || stripos($sfile->file($_FILES[$k]['tmp_name'][$f]), 'htaccess') !== false) {
